...
const initialState = {
    ...
    },
    {
      ...
      price: 2,
      category: "Drink",
      image: "https://cdn-icons-png.flaticon.com/512/4931/4931276.png",
    },
    {
      id: uuidv4(),
    ...
    },
    {
      ...
      price: 1,
      category: "Drink",
      image: "https://cdn-icons-png.flaticon.com/512/1349/1349838.png",
    },
  ],
  cartItems: [],
  category: "",
...
const reducer = ( state, action ) => {
  switch (action.type) {
    ...
    case 'ADD_CART':
      let updatedCart = [];
      const targetItem = state.cartItems.filter((item) => item.id === action.payload.id) 
    ...
    case 'DELETE_LIST':
      ...
      const deleteList2 = state.cartItems.filter((item) => item.id !== action.payload.id) 
    ...
    case 'UPDATE_QUANTITY': 
      ...
      if (action.payload.method === 'INCREMENT') {
        ...
        if (removeFlag == true) {
          updatedCarts = updatedCarts.filter((item) => item.id !== action.payload.id);
        }
      } 
    ...
    case 'EDIT_ITEM_DETAIL':
      const itemDetails = state.items.filter((item) => item.id === action.payload.id);
      return {...state, editItemDetail: itemDetails, editItem: action.payload.flag};
    case 'SAVE_CHANGES' :
      ...
      return {...state, items: updatedDetail, editItem: false, cartItems: updatedCartItems};
...
const App = () => {
  ...
  const listItems =
    ...
    const showAddItemForm = () => {
      state.newItem ? dispatch({ type: 'TOGGLE_NEW_ITEM', payload: {flag: false}}) : dispatch({ type: 'TOGGLE_NEW_ITEM', payload: {flag: true}});
    }; 

    const hideNewItemForm = (status) => {
      state.editItem ? dispatch({ type: 'TOGGLE_NEW_ITEM', payload: {flag: status}}) : dispatch({ type: 'TOGGLE_NEW_ITEM', payload: {flag: status}});
    };

    const hideEditItemForm = (status) => {
    state.editItem ? dispatch ({ type: 'TOGGLE_EDIT_ITEM', payload: {flag: status}}) : dispatch ({ type: 'TOGGLE_EDIT_ITEM', payload: {flag: status}});
    };
  ...
  return (
    <>
      ...
      <h1>Cart Total Amount: $ <u>{totalAmount}</u></h1>
